...
MeasurementHelper.toTemperature = (temperature, unit = Constants.UNIT_SI, digits = 1, withUnit = true) => unit === Constants.UNIT_SI ? `${NumberFormatHelper.toFixed(temperature, digits)}${withUnit ? '°C' : ''}` : `${NumberFormatHelper.toFixed(temperature * 1.8 + 32, digits)}${withUnit ? '°F' : ''}`;
